...
file_log  <- paste('DMA-T_',inizio1,'_',fine1,'_rem2.log',sep='')
...
#==============================================================================
cat ( "ESECUZIONE DMA-T ", date()," \n\n" , file = file_log)
cat ( " tipologia > Termometri" , file = file_log,append=T)
cat ( "\n" , file = file_log,append=T)
...
query_coppie <- paste ("select distinct IDsensore, Data_e_ora from ",nome_tavola_DQC," where Data_e_ora>'",inizio,"' and Data_e_ora<'",fine ,"' and ( (Test='P1a' and Result='F')  or Test='S1a' or Test='T1a' or (Test='T2a' and Result='F') )", sep="")
#print(query_coppie)
...
#..............................................
  ...
  auxS1aF <- any( (q_esito$Test %in% 'S1a') & (q_esito$Result %in% 'F') )
  auxS1aS <- any( (q_esito$Test %in% 'S1a') & (q_esito$Result %in% 'S') )
  auxT1aF <- any( (q_esito$Test %in% 'T1a') & (q_esito$Result %in% 'F') )
  auxT1aS <- any( (q_esito$Test %in% 'T1a') & (q_esito$Result %in% 'S') )
  auxT2aF <- any( (q_esito$Test %in% 'T2a') & (q_esito$Result %in% 'F') )
# DMA-T-4
  if( (auxS1aS == TRUE) & (auxT1aS == TRUE) ) flag='F'
# DMA-T-5
  if( (auxP1aF == TRUE) ) flag='F'
# DMA-T-6
  if( (auxS1aF == TRUE) ) flag='F'
# DMA-T-7
  if( (auxT2aF == TRUE) ) flag='F'
#
  cat( paste(" Risultati: P1a F? S1a F? S1a S? T1a F? T1a S? T2a F? ",auxP1aF,auxS1aF,auxS1aS,auxT1aF,auxT1aS,auxT2aF,'allora risultato finale =',flag,"\n"), file=file_log, append=T)

#___________________________________________________
# prima di scrivere nelle tabelle dati l'esito dei test 
# verifico il valore della flag_automatica dell'ultimo update. 
...
#___________________________________________________
  for (anno in anni) {
    ...
    query_update_annuale <- paste ("update ", nome_tavola_annuale ," set Flag_automatica='",flag, "', Autore='DMA-T.R',Data='",Sys.time(),"' where IDsensore=",q_coppie$IDsensore[coppia], " and Data_e_ora='",q_coppie$Data_e_ora[coppia],"'"  , sep="")

    q_update_annuale <- try( dbGetQuery(conn,query_update_annuale), silent=TRUE )
    if (inherits( q_update_annuale, "try-error")) {
      quit(status=1)
    }
    query_update_annuale_bis <- paste ("update ", nome_tavola_annuale ," set Flag_manuale_DBunico=5, Autore='DMA-T.R',Data='",Sys.time(),"' where IDsensore=",q_coppie$IDsensore[coppia], " and Data_e_ora='",q_coppie$Data_e_ora[coppia],"' and Flag_automatica='F' and Flag_manuale_DBunico in (-1,0,1,2)"  , sep="")

    q_update_annuale_bis <- try( dbGetQuery(conn,query_update_annuale_bis), silent=TRUE )
    if (inherits( q_update_annuale_bis, "try-error")) {
      quit(status=1)
    }
    query_update_annuale_tris <- paste ("update ", nome_tavola_annuale ," set Flag_manuale_DBunico=-1, Autore='DMA-T.R',Data='",Sys.time(),"' where IDsensore=",q_coppie$IDsensore[coppia], " and Data_e_ora='",q_coppie$Data_e_ora[coppia],"' and Flag_automatica='P' and Flag_manuale_DBunico in (0,5)"  , sep="")
...
# la query tris allinea le Flag_manuale_DBunico in caso di P
#___________________________________________________
  query_update_recente <- paste ("update ", nome_tavola_recente ," set Flag_automatica='",flag, "', Autore='DMA-T.R',Data='",Sys.time(),"' where IDsensore=",q_coppie$IDsensore[coppia], " and Data_e_ora='",q_coppie$Data_e_ora[coppia],"'"  , sep="")

  q_update_recente <- try( dbGetQuery(conn,query_update_recente), silent=TRUE )
  if (inherits( q_update_recente, "try-error")) {
    quit(status=1)
  }
  query_update_recente_bis <- paste ("update ", nome_tavola_recente ," set Flag_manuale_DBunico=5, Autore='DMA-T.R',Data='",Sys.time(),"' where IDsensore=",q_coppie$IDsensore[coppia], " and Data_e_ora='",q_coppie$Data_e_ora[coppia],"' and Flag_automatica='F' and Flag_manuale_DBunico in (-1,0,1,2)"  , sep="")
  ...
  query_update_recente_tris <- paste ("update ", nome_tavola_recente ," set Flag_manuale_DBunico=-1, Autore='DMA-T.R',Data='",Sys.time(),"' where IDsensore=",q_coppie$IDsensore[coppia], " and Data_e_ora='",q_coppie$Data_e_ora[coppia],"' and Flag_automatica='P' and Flag_manuale_DBunico in (0,5)"  , sep="")
