...
#
# Riga di comando:
# R --vanilla inizio fine < DMA-DV.R
...
file_log  <- paste('DMA-DV_',inizio1,'_',fine1,'_rem2.log',sep='')
...
#==============================================================================
cat ( "ESECUZIONE DMA-DV ", date()," \n\n" , file = file_log)
cat ( " tipologia > AnemometriDV" , file = file_log,append=T)
cat ( "\n" , file = file_log,append=T)
...
#
  cat( paste(" Risultati: P1a F? T2a S? ",auxP1aF,auxT2aS,'allora risultato finale =',flag,"\n"),
       file=file_log, append=T)
...
#___________________________________________________
  for (anno in anni) {
    ...
    query_update_annuale <- paste ("update ", nome_tavola_annuale ," set Flag_automatica='",flag, "', Autore='DMA-DV.R',Data='",Sys.time(),"' where IDsensore=",q_coppie$IDsensore[coppia], " and Data_e_ora='",q_coppie$Data_e_ora[coppia],"'"  , sep="")

    q_update_annuale <- try( dbGetQuery(conn,query_update_annuale), silent=TRUE )
    if (inherits( q_update_annuale, "try-error")) {
      quit(status=1)
    }
    query_update_annuale_bis <- paste ("update ", nome_tavola_annuale ," set Flag_manuale_DBunico=5, Autore='DMA-DV.R',Data='",Sys.time(),"' where IDsensore=",q_coppie$IDsensore[coppia], " and Data_e_ora='",q_coppie$Data_e_ora[coppia],"' and Flag_automatica='F' and Flag_manuale_DBunico in (-1,0,1,2)"  , sep="")

    q_update_annuale_bis <- try( dbGetQuery(conn,query_update_annuale_bis), silent=TRUE )
    if (inherits( q_update_annuale_bis, "try-error")) {
      quit(status=1)
    }
    query_update_annuale_tris <- paste ("update ", nome_tavola_annuale ," set Flag_manuale_DBunico=-1, Autore='DMA-DV.R',Data='",Sys.time(),"' where IDsensore=",q_coppie$IDsensore[coppia], " and Data_e_ora='",q_coppie$Data_e_ora[coppia],"' and Flag_automatica='P' and Flag_manuale_DBunico in (0,5)"  , sep="")
...
# la query tris allinea le Flag_manuale_DBunico in caso di P
#___________________________________________________
  query_update_recente <- paste ("update ", nome_tavola_recente ," set Flag_automatica='",flag, "', Autore='DMA-DV.R',Data='",Sys.time(),"' where IDsensore=",q_coppie$IDsensore[coppia], " and Data_e_ora='",q_coppie$Data_e_ora[coppia],"'"  , sep="")

  q_update_recente <- try( dbGetQuery(conn,query_update_recente), silent=TRUE )
  if (inherits( q_update_recente, "try-error")) {
    quit(status=1)
  }
  query_update_recente_bis <- paste ("update ", nome_tavola_recente ," set Flag_manuale_DBunico=5, Autore='DMA-DV.R',Data='",Sys.time(),"' where IDsensore=",q_coppie$IDsensore[coppia], " and Data_e_ora='",q_coppie$Data_e_ora[coppia],"' and Flag_automatica='F' and Flag_manuale_DBunico in (-1,0,1,2)"  , sep="")
  ...
  query_update_recente_tris <- paste ("update ", nome_tavola_recente ," set Flag_manuale_DBunico=-1, Autore='DMA-DV.R',Data='",Sys.time(),"' where IDsensore=",q_coppie$IDsensore[coppia], " and Data_e_ora='",q_coppie$Data_e_ora[coppia],"' and Flag_automatica='P' and Flag_manuale_DBunico in (0,5)"  , sep="")
