...
#
# Riga di comando:
# R --vanilla inizio fine < DMA-UR.R
...
file_log  <- paste('DMA-UR_',inizio1,'_',fine1,'_rem2.log',sep='')
tipologia<-"Igrometri"
#
...
#==============================================================================
cat ( "ESECUZIONE DMA-UR ", date()," \n\n" , file = file_log)
cat ( " tipologia > Igrometri" , file = file_log,append=T)
cat ( "\n" , file = file_log,append=T)
...
query_coppie <- paste ("select distinct IDsensore, Data_e_ora from ",nome_tavola_DQC," where Data_e_ora>'",inizio,"' and Data_e_ora<'",fine ,"' and ( (Test='P1a' and Result='F')  or (Test='T2b' and Result='F') or (Test='T2a' and Result='F') )", sep="")
...
#..............................................
  ...
  auxT2bF <- any( (q_esito$Test %in% 'T2b') & (q_esito$Result %in% 'F') )
  auxT2aF <- any( (q_esito$Test %in% 'T2a') & (q_esito$Result %in% 'F') )
# DMA-UR-5
  if( (auxP1aF == TRUE) ) flag='F'
# DMA-UR-6
  if( (auxT2bF == TRUE) ) flag='F'
# DMA-UR-7
  if( (auxT2aF == TRUE) ) flag='F'
#
  cat( paste(" Risultati: P1a F? T2b F? T2a F? ",auxP1aF,auxT2bF,auxT2aF,'allora risultato finale =',flag,"\n"),
       file=file_log, append=T)
...
#___________________________________________________
  for (anno in anni) {
    ...
    query_update_annuale <- paste ("update ", nome_tavola_annuale ," set Flag_automatica='",flag, "', Autore='DMA-UR.R',Data='",Sys.time(),"' where IDsensore=",q_coppie$IDsensore[coppia], " and Data_e_ora='",q_coppie$Data_e_ora[coppia],"'"  , sep="")

    q_update_annuale <- try( dbGetQuery(conn,query_update_annuale), silent=TRUE )
    if (inherits( q_update_annuale, "try-error")) {
      quit(status=1)
    }
    query_update_annuale_bis <- paste ("update ", nome_tavola_annuale ," set Flag_manuale_DBunico=5, Autore='DMA-UR.R',Data='",Sys.time(),"' where IDsensore=",q_coppie$IDsensore[coppia], " and Data_e_ora='",q_coppie$Data_e_ora[coppia],"' and Flag_automatica='F' and Flag_manuale_DBunico in (-1,0,1,2)"  , sep="")

    q_update_annuale_bis <- try( dbGetQuery(conn,query_update_annuale_bis), silent=TRUE )
    if (inherits( q_update_annuale_bis, "try-error")) {
      quit(status=1)
    }
    query_update_annuale_tris <- paste ("update ", nome_tavola_annuale ," set Flag_manuale_DBunico=-1, Autore='DMA-UR.R',Data='",Sys.time(),"' where IDsensore=",q_coppie$IDsensore[coppia], " and Data_e_ora='",q_coppie$Data_e_ora[coppia],"' and Flag_automatica='P' and Flag_manuale_DBunico in (0,5)"  , sep="")
...
# la query tris allinea le Flag_manuale_DBunico in caso di P
#___________________________________________________
  query_update_recente <- paste ("update ", nome_tavola_recente ," set Flag_automatica='",flag, "', Autore='DMA-UR.R',Data='",Sys.time(),"' where IDsensore=",q_coppie$IDsensore[coppia], " and Data_e_ora='",q_coppie$Data_e_ora[coppia],"'"  , sep="")

  q_update_recente <- try( dbGetQuery(conn,query_update_recente), silent=TRUE )
  if (inherits( q_update_recente, "try-error")) {
    quit(status=1)
  }
  query_update_recente_bis <- paste ("update ", nome_tavola_recente ," set Flag_manuale_DBunico=5, Autore='DMA-UR.R',Data='",Sys.time(),"' where IDsensore=",q_coppie$IDsensore[coppia], " and Data_e_ora='",q_coppie$Data_e_ora[coppia],"' and Flag_automatica='F' and Flag_manuale_DBunico in (-1,0,1,2)"  , sep="")
  ...
  query_update_recente_tris <- paste ("update ", nome_tavola_recente ," set Flag_manuale_DBunico=-1, Autore='DMA-UR.R',Data='",Sys.time(),"' where IDsensore=",q_coppie$IDsensore[coppia], " and Data_e_ora='",q_coppie$Data_e_ora[coppia],"' and Flag_automatica='P' and Flag_manuale_DBunico in (0,5)"  , sep="")
